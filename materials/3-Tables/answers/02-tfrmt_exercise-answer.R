...
ard_demog2 <- ard_demog |>
  mutate(label = case_when(
    ...
    stat_name %in% c("median","p25","p75") ~ "Median (Q1, Q3)", # b1) stat_name "median", "p25", and "p75" to have the same label "Median (Q1, Q3)"
    TRUE ~ label
  ))
...
tfrmt_demog <- tfrmt(
  ...
  body_plan = body_plan(
    ...
    frmt_structure(
      group_val = ".default", label_val = ".default",
      frmt_combine(
        expression = "{mean} ({sd})", # a1) complete expression to display as `mean (sd)`
        mean = frmt("xx.xx"),         # a1) complete `frmt` so values are rounded to 2 dec places
        sd = frmt("xx.xx")            # a1) complete `frmt` so values are rounded to 2 dec places
      )
    ),
    frmt_structure(
      group_val = ".default", label_val = ".default",
      frmt_combine(
        expression = "{median} ({p25}, {p75})", # a2) complete expression to display as `median (p25, p75)`
        median = frmt("xx.x"),                  # a2) complete `frmt` so values are rounded to 1 dec place
        p25 = frmt("xx.x"),                     # a2) complete `frmt` so values are rounded to 1 dec place
        p75 = frmt("xx.x")                      # a2) complete `frmt` so values are rounded to 1 dec place
      )
    )
  ) ,
  ...
  footnote_plan = footnote_plan(
    footnote_structure("A footnote for Placebo", column_val = "Placebo")  # c) complete the column val
  )
)
