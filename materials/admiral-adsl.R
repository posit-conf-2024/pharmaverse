...
###USING TIDYVERSE
#would use ymd_hms if the dtc column contained time values - for this example used hour/min/sec to add EOD time
ex_ext2 <- ex %>% mutate(EXSTDTM =  ymd(EXSTDTC),
                         EXENDTM = ymd(EXENDTC) + hours(23) + minutes(59) + seconds(59))
...
###USING TIDYVERSE
#in order to merge in the ex domain to adsl and do SAFFL flag - this is a route you would have to take 
ex_ext2 <- ex_ext %>% select(STUDYID, USUBJID, EXDOSE, EXTRT, EXSTDTM) %>% 
  filter((EXDOSE > 0 |
            (EXDOSE == 0 &
               str_detect(EXTRT, "PLACEBO"))) &
           !is.na(EXSTDTM)) %>% 
  mutate(SAFFL = ifelse((EXDOSE > 0 | (EXDOSE == 0 & str_detect(EXTRT, "PLACEBO"))), "Y", NA)) %>%
  select(-EXSTDTM, -EXDOSE,-EXTRT)

adsl2 <- adsl2 %>% left_join(ex_ext2)

#example for calculating TRTDURD instead of simplicity of derive_var_trtdurd() function
adsl2 <- adsl2 %>% mutate(TRTDURD = as.numeric(difftime(TRTEDTM, TRTSDT, units = "days")),
                          TRTDURD = ceiling(TRTDURD))
